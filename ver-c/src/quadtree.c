...
int quadrant_pos(double pos[2], double p_pos[2]){
  int quad_index;
  quad_index = 0;
  if (pos[0] >= p_pos[0]){ quad_index++; }
  if (pos[1] <= p_pos[1]){ quad_index += 2; }
  return quad_index;
}
...
void split_cell(){
  ...
  lifo_index = malloc(sizeof(int)*qtree.num_nodes);
  ...
  while(1){
    ...
    if (p_num_vor > 0){
      for (i=0;i<4;i++){
        qtree.node[c_index[i]].vor_id = malloc(sizeof(int) * p_num_vor);
        qtree.node[c_index[i]].num_vor = 0;
    ...
    for (i=0; i<4; i++){
      ...
      qtree.node[c_index[i]].outer_coeffs = malloc(sizeof(double complex)*qtree.nterms);
